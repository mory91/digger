...
def get_with_maxlen(trace, start_idx, max_len):
    return np.array(
        [np.pad(
            np.array(trace[s:s + max_len]),
            (0, max(0, max_len - len(trace[s:s + max_len]))))
            for s in start_idx]
    )
...
def get_flow_trace(trace, time_delta, max_len=MAX_LEN):
    ...
    start_idx = flows[:, 0]
    flows_sizes_trace = get_with_maxlen(value_trace, start_idx, max_len)
    flows_times_trace = get_with_maxlen(time_stamp, start_idx, max_len)
