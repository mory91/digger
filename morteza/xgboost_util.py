...
def print_metrics(real, prediction, problem_type=REGRESSION, emthresh=3500):
    if problem_type == REGRESSION:
        # TVD=0, ACCURACY=1, ROC=2, F1=3, REG_TVD=4, MSE=5, MAE=6, R2=7
        mse = mean_squared_error(real, prediction)
        mae = mean_absolute_error(real, prediction)
        r2 = r2_score(real, prediction)
        reg_tvd = tvd(real, prediction)

        scores = (reg_tvd, mse, mae, r2)

        # See the classification score with regression model
        true_class = (np.array(real) > emthresh) * 1
        model_class = (np.array(prediction) > emthresh) * 1
        model_scores = (
            tvd(true_class, model_class),
            accuracy_score(true_class, model_class),
            roc_auc_score(true_class, model_class),
            f1_score(true_class, model_class)
        )

        scores = model_scores + scores

    elif problem_type == CLASSIFICATION:
        # TVD=0, ACCURACY=1, ROC=2, F1=3
        pred = np.round(prediction)
        scores = (
            tvd(real, pred),
            accuracy_score(real, pred),
            roc_auc_score(real, prediction),
            f1_score(real, pred)
        )
