...
def parse_packets_file(file_name):
    ...
    df = df.apply(pd.to_numeric, errors='coerce')
    df = df.dropna()
    df_egress = df[df['dir'] == EGRESS][["timestamp", "size"]]
    df_ingress = df[df['dir'] == INGRESS][["timestamp", "size"]]
...
def make_tmp_flows(prefix=DEFAULT_PREFIX):
    ...
    trace_names = [DISK_READ, DISK_WRITE, MEMORY, ALLOCATIONS, RSS_MEMORY, DATA_MEMORY, U_TIME, S_TIME, SENDS]
    ...
    trace_names = [DISK_READ, DISK_WRITE, MEMORY, PACKETS_EGRESS, PACKETS_INGRESS, ALLOCATIONS, RSS_MEMORY, DATA_MEMORY, U_TIME, S_TIME] #, TARGET_PACKETS_EGRESS, TARGET_PACKETS_INGRESS, SENDS]
